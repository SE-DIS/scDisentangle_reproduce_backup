...
yaml_path = "configs/ici.yaml"
max_epochs = 521

custom_name = yaml_path.split("/")[-1].split(".")[0]
data_name = "ici"

patients = ['P1','P3','P20','P9','P11','P10','P4','P5']
...
def run_single_job(args):
    ...
    hparams["OOD"]["filter_dict"]["patient_id"] = ood_cov
    hparams["wandb"]["name"] += f"{ood_cov}_{seed_nb}"
    hparams["train"]["set_seed"] = seed_nb
    # hparams["growing_neurons"]["prior_mappers"]["mappers"]["patient_id_mapper"][
    #     "collapse_name"
    # ] = ood_cov
    ...
    train_scdisentangle.make_run(
        ...
        seed_nb=seed_nb,
        save_path=f"{job_dir}/",
        split_key=None,#f'split_On treatment_{ood_cov}',
    )
    
if __name__ == "__main__":
    # Use the same seed as in train.py
    seed_nbs = list(range(1, 6))
    ...
    jobs = []
    for i1, cov in enumerate(patients):
